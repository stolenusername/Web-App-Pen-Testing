...
const (
	HTTPPort        = 80
	DNSPort         = 53
	DNSResponseIP   = "45.79.11.176"      //modify if needed, this is Yolo's IP
	DNSResponseName = "webhacking.ninja." //Change to the desired domain name
	DefaultTTL      = 3600
)
			...
			if r.Question[0].Qtype == dns.TypeNS {
				response.Answer = append(response.Answer,
					&dns.NS{
						...
						Ns:  "ns1.webhacking.ninja.", //Change to the desired name servers
					},
					&dns.NS{
						Hdr: dns.RR_Header{Name: DNSResponseName, Rrtype: dns.TypeNS, Class: dns.ClassINET, Ttl: DefaultTTL},
						Ns:  "ns2.webhacking.ninja.", //Change to the desired name servers
					})
